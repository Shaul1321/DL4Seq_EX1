...
def sanity_check():
    ...
    like_relu = lambda x: (np.sum(np.maximum(x,0)), np.where(x>0., 1, 0))
    ...
    gradient_check(like_relu, np.array(123.456))      # scalar test
    gradient_check(like_relu, np.random.randn(3,))    # 1-D test
    gradient_check(like_relu, np.random.randn(4,5))   # 2-D test
    print ""
